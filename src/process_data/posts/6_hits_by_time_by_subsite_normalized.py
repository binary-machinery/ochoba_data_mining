    ...
    x, y = db.fetch_data(
        """
            with time_scale as (
                ...
                from generate_series('2018-06-06'::timestamp, '2020-07-20'::timestamp, '1 week'::interval)
                order by time_window
            )
            select time_scale.time_window, coalesce(data.sum, 0) as count
            from time_scale
            left join (
                select date_trunc('week', created) as time_window, sum(hits_count)
                from posts
